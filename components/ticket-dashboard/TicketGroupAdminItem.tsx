...
import TicketGroupDetail from "../settings/TicketGroup/TicketGroupDetail";
...
const TicketGroupAdminItem: React.FC<Props> = (props) => {
				...
				<Stack spacing={2} direction="row" sx={{ justifyContent: "center" }}>
					...
					{nextQueueDelay && (
						<Button onClick={aboartQueueHandler} sx={{ fontWeight: "bolder", flex: 1 }} variant="contained" color={currentQueue ? "error" : "error"}>
							...
							{currentQueue && waitingQueueList.length > 0 && lang.ticket.button.finishAndNext}
							{currentQueue && waitingQueueList.length == 0 && lang.ticket.button.finish} ({delayRemain})
						</Button>
					)}
					{currentQueue && (
						<Button
	...
	return (
		<>
			<Card sx={{ width: cardSize, maxWidth:cardSize, mr:{xs:0,sm:2}, mb:3 }}>
				<TicketGroupDetail open={showDetail} ticketGroup={ticketGroup} onClose={closeDetailHandler} />
				<ConfirmDialog open={confirmActive} confirmLabel={lang.admin.ticketGroup.status.active} onConfirm={activeTicketGroupHandler} onReject={activeTicketGroupClose} />
				<ConfirmDialog open={confirmInactive} confirmLabel={lang.admin.ticketGroup.status.inactive} onConfirm={inactiveTicketGroupHandler} onReject={inactiveTicketGroupClose} />
				<CardContent sx={{ paddingBottom: 0 }}>
					<Box component="div" sx={{ marginBottom: 1 }}>
						{ticketGroup.active === 0 && (
							<Button onClick={activeTicketGroupConfirm} color="error" variant="contained" startIcon={<RadioButtonUncheckedIcon />} fullWidth>
								{lang.admin.ticketGroup.status.inactive}
							</Button>
						)}
						{ticketGroup.active === 1 && (
							<Button onClick={inactiveTicketGroupConfirm} color="success" variant="contained" startIcon={<RadioButtonCheckedIcon />} fullWidth>
					...
					<Box component="div" sx={{ margin: 0 }}>
						<Grid container spacing={2}>
							<Grid item md={4} xs={12}>
								<Button onClick={openDetailHandler} variant="text" disabled={ticketGroup.active === 0} fullWidth>

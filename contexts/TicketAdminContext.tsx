...
type TicketAdminContextObj = {
	...
	poseponeTicket: (ticketId: number) => Promise<Ticket | false>;
...
export const TicketAdminContext = React.createContext<TicketAdminContextObj>({
	...
	poseponeTicket: async (ticketId: number) => false,
	rejectTicket: async (ticketId: number) => false,
});

const TicketAdminContextProvider: React.FC<OriginProps> = (props) => {
	...
	const poseponeTicket = async (ticketId: number): Promise<Ticket | false> => {
		return false;
	};
